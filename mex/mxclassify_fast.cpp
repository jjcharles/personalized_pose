...
// FAST because we use interlacing
#include "mex.h"
#include <stdlib.h>
#include <time.h>
#include "applytree.h"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    const mxArray* apInterlaceflag = prhs[8];
    
    int numclasses = mxGetScalar(apNumclasses);
    int interlaceflag = mxGetScalar(apInterlaceflag);
    ...
    for (int i = 0; i<(box_width*box_height); i++) {
        ...
        if (((interlaceflag==1) && (i%2 == 0)) || (((interlaceflag==0) && (i%2 == 1)))){
            ...
            for (int c = 0; c<numclasses; c++) {
                output[i + (int)(c*(box_height*box_width))] = 0;
                leafid[i] = -1;
            }
        }
    }
