...
template.width = ceil(opts.evaluator.hog.shape.width/opts.evaluator.hog.cellsize)-2;
template.height =ceil(opts.evaluator.hog.shape.height/opts.evaluator.hog.cellsize)-2;
template.img = zeros(template.height,template.width,3,'uint8');

    
if visualise
    figure(100);
    subplot(1,2,1)
    h_img_left = imagesc(template.img);
    colormap gray
    subplot(1,2,2)
    h_img_right = imagesc(template.img);
    colormap gray
end
num_negperframe = 2;
numpos = 4;
len_data = template.width*template.height*32;
total_train = 2*num_negperframe*total_frames + numpos*total_frames;
...
for i = 1:total_frames
    ...
    fprintf('training lower arm HOG evaluator on %s detections, frame %d of %d\n',fieldname,i,total_frames);
    ...
    if locs(1,3,i)~=-999
        ...
        left_shape = get_positives(opts,imgorig,locs(:,[3 5],i)',left_shape,numpos,pos_samplewidth);  
    ...
    if locs(1,2,i)~=-999
        ...
        right_shape = get_positives(opts,imgorig,locs(:,[2 4],i)',right_shape,numpos,pos_samplewidth);       
    ...
    if visualise
        ...
        avg_left_hog = reshape(avg_left_hog,[template.height,template.width,32]);
        ...
        avg_right_hog = reshape(avg_right_hog,[template.height,template.width,32]);
...
%function to add hog patch to append hog patch to the training set
    function shape_model = append_patch(opts,img,locs,shape_model,label)
        can_patch = shape.img2can(img,locs(:,1),locs(:,2),opts.shape.anchor,opts.evaluator.hog.shape.width,opts.evaluator.hog.shape.height,'rgb');
        hogpatch = single(features.hog(can_patch,opts.evaluator.hog.cellsize));
        shape_model.traindata(shape_model.count,:) = hogpatch(:)';
...
%function to grab positive patches from an image
    function shape_model = get_positives(opts,img,locs,shape_model,numpos,pos_samplewidth)
        
        [M,N,~] = size(img);
        for n = 1:numpos
            %get patch distorted from wrist
            randx = floor(rand*pos_samplewidth) + 1 + locs(1,1)-ceil(pos_samplewidth/2);
            randy = floor(rand*pos_samplewidth) + 1 + locs(1,2)-ceil(pos_samplewidth/2);            
            randx(randx<1) = 1;
            randy(randy<1) = 1;
            randx(randx>N) = N;
            randy(randy>M) = M;
            ...
            %get patch distorted from elbow
            randx = floor(rand*pos_samplewidth) + 1 + locs(2,1)-ceil(pos_samplewidth/2);
            randy = floor(rand*pos_samplewidth) + 1 + locs(2,2)-ceil(pos_samplewidth/2);
            
            randx(randx<1) = 1;
            randy(randy<1) = 1;
            randx(randx>N) = N;
            randy(randy>M) = M;
            templocs = [templocs(1,:);[randx,randy]];
            shape_model = append_patch(opts,img,templocs,shape_model,1); %use a positive label (1)    
        end
